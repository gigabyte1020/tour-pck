...
<script>
 ...
 <a name="active"></a>
                                            ...
                                            ConnectionCls obj=new ConnectionCls();
                                             ResultSet rs,rs5;
                                            String qry="SELECT * FROM `c_register` WHERE `role`='guide' and `c_status`='null'";
                                           //
                                         //  String qry="SELECT * FROM `c_register` WHERE `c_status` !='Reject' and role='guide'";
                                           ...
                                           if(s.matches("0"))
                                                      {
                                                        ...
                                                        %>
                                                        <td><a href="admin_guide_approve.jsp?id_c=<%=rs.getString("c_id")%>&name=<%=rs.getString("c_name")%>"><span class="glyphicon glyphicon-ok " title="Approve"></span></a>
                                                           
                                                            <a href="admin_guide_reject.jsp?id_c=<%=rs.getString("c_id")%>&name=<%=rs.getString("c_name")%>"><span class="glyphicon glyphicon-remove" title="Reject"></span></a></td> 
                                                       <%
                                                                }
...
%>
                                                        ...
                                                        <td ><a href="admin_guide_reject.jsp?id_c=<%=rs.getString("c_id")%>&name=<%=rs.getString("c_name")%>"><span class="glyphicon glyphicon-trash"title="Suspend"></span></a>                                           
                                                            <a href="adminviewrequest.jsp?id_c=<%=rs.getString("c_id")%>&name=<%=rs.getString("c_name")%>&lo_id=<%=rs.getString("log_id")%>"><span class="glyphicon glyphicon-file"title="View Openings"></span></a>
                                                          <%
                                                          if(rs5.next())
{
